...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        ...
        self.pos_query = nn.Linear(hidden_size, self.all_head_size)
        self.pos_key = nn.Linear(hidden_size, self.all_head_size)

        self.fusion_layer = VanillaAttention(50,50)
    ...
    def forward(self, input_tensor, position_embedding, attention_mask):
        ...
        pos_query_layer = self.transpose_for_scores(self.pos_query(position_embedding))
        pos_key_layer = self.transpose_for_scores(self.pos_key(position_embedding))
        pos_attention_scores = torch.matmul(pos_query_layer, pos_key_layer.transpose(-1, -2))

        # Take the dot product between "query" and "key" to get the raw attention scores.
        item_attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        # attention_scores = item_attention_scores + pos_attention_scores
        attention_scores = torch.cat([item_attention_scores.unsqueeze(-2), pos_attention_scores.unsqueeze(-2)], dim=-2)
...
class DIFMultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size,attribute_hidden_size,feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps,fusion_type,max_len):
        ...
        self.query_layers = nn.ModuleList([copy.deepcopy(nn.Linear(hidden_size, self.all_head_size)) for _ in range(self.feat_num)])
        self.key_layers = nn.ModuleList(
            [copy.deepcopy(nn.Linear(hidden_size, self.all_head_size)) for _ in range(self.feat_num)])
        self.value_layers = nn.ModuleList(
            [copy.deepcopy(nn.Linear(hidden_size, self.all_head_size)) for _ in range(self.feat_num)])

        self.concat_layer = nn.Linear(hidden_size * (1 + self.feat_num), hidden_size)
    ...
    def compute(self, input_tensor, value_layer, attention_scores, attention_mask):
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
    ...
    def forward(self, input_tensor,attribute_table,position_embedding, attention_mask):
        ...
        for i, (attribute_query, attribute_key, attribute_value) in enumerate(
            ...
            attribute_hidden_states = self.compute(attribute_tensor, attribute_value_layer, attribute_attention_scores, attention_mask)
        ...
        if self.fusion_type == 'sum':
            ...
            # attention_scores = torch.cat(
            #     [attribute_attention_table, item_attention_scores.unsqueeze(-2)], dim=-2)
            # attention_scores = torch.cat(
            #     [item_attention_scores.unsqueeze(-2), pos_scores.unsqueeze(-2)], dim=-2)
            attention_scores,_ = self.fusion_layer(attention_scores)

        item_hidden_states = self.compute(input_tensor,item_value_layer, attention_scores, attention_mask)
...
class DIFTransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states,attribute_embed,position_embedding, attention_mask):
        attention_output, attribute_attention_output = self.multi_head_attention(hidden_states,attribute_embed,position_embedding, attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        feedforward_attribute_output = []
        for output in attribute_attention_output:
            feedforward_attribute_output.append(self.feed_forward(output))        
