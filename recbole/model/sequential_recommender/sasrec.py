...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        ...
        self.subln = RMSNorm(self.attention_head_size, eps=1e-5, elementwise_affine=True)
    ...
    def forward(self, input_tensor, attention_mask, att_map=None):
        ...
        if att_map is not None:
            return hidden_states, attribute_map
        else:
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, att_map=None):
        if att_map is not None:
            hidden_states, att_map_l = self.multi_head_attention(hidden_states, attention_mask, att_map)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, att_map=None, output_all_encoded_layers=True):
        ...
        if att_map is not None:
            ...
            for i in range(att_map_l.shape[0]):
                att_map.append(att_map_l[i].detach().cpu().tolist())
...
class SASRec(SequentialRecommender):
    ...
    def bpr_loss_with_label_smoothing(self, pos_scores, neg_scores):
        ...
        # For wandb logging - tracking log(softmax(scores))
    ...
    def get_avg_log_scores(self):
        """Get average log(softmax(scores)) for the epoch."""
    ...
    def get_attention_mask(self, item_seq):
        ...
        # Create tensor directly on the device for MPS compatibility
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            if self.label_smoothing > 0:
                loss = self.bpr_loss_with_label_smoothing(pos_score, neg_score)
            else:
            ...
            # Track log(softmax(scores)) for wandb logging
            with torch.no_grad():
                import torch.nn.functional as F
                log_probs = F.log_softmax(logits, dim=1)
                batch_size = pos_items.shape[0]
                log_pos = log_probs[torch.arange(batch_size, device=pos_items.device), pos_items].mean().item()
                # For neg, sample random items (excluding pos)
                rand_neg = torch.randint(1, self.n_items, (batch_size,), device=pos_items.device)
                log_neg = log_probs[torch.arange(batch_size, device=pos_items.device), rand_neg].mean().item()
